...
const SLIDES = [
  ...
  { id: "sarah",       label: "Capabilities",     shortLabel: "6" },
  { id: "tidbx",       label: "TiDB Cloud",       shortLabel: "7" },
  { id: "byoc",        label: "BYOC",             shortLabel: "8" },
  { id: "memories",    label: "Three Memories",   shortLabel: "9" },
  { id: "maya",        label: "Maya's Year",      shortLabel: "10" },
  { id: "librarian",   label: "Token Tax",        shortLabel: "11" },
  { id: "manus",       label: "AI-Native Threat", shortLabel: "12" },
  { id: "whynow",      label: "Closing",          shortLabel: "13" }
];
...
const SPEAKER_NOTES = {
  ...
  sarah:
    "[~3 min] THE CAPABILITIES MAP. Names the four products the audience would otherwise buy and shows the equivalent TiDB feature for each. The point: this isn't 'TiDB has vectors too' - this is 'four budget lines collapsed into one operational layer.'\n\nOPEN: 'Every product on the left is a real budget line in your org. Every capability on the right is built into TiDB.'\n\nWALK THE ROWS - keep it crisp, ~25 sec each:\n\nROW 1 - Pinecone / Weaviate / pgvector -> Vector Search\n   - 'Native HNSW index, up to 16,383 dimensions. Same SQL connection. No second database.'\n\nROW 2 - Elasticsearch / OpenSearch -> Full-Text Search (BM25)\n   - 'Tantivy-backed ranking, runs on text and JSON. Your search team just got a fewer system to operate.'\n\nROW 3 - MySQL / Postgres / Aurora -> SQL + JOINs\n   - 'MySQL wire protocol. Distributed ACID. Your existing apps don't know they're talking to a distributed system.'\n   - This is the row that anchors the spine: TiDB doesn't replace your MySQL fluency - it consolidates the systems around it.\n\nROW 4 - LangChain / custom Python -> Hybrid Search + Auto Embedding\n   - 'Vector and keyword in one query. Embeddings generated automatically on insert. The orchestration layer your team would have built and maintained - it's a SQL function.'\n   - This is the highest-value row for engineering CIOs - the glue code that breaks at scale never gets written.\n\nROW 5 - Snowflake / BigQuery -> Columnar analytics (TiFlash)\n   - 'Same data, columnar replica. Zero ETL. Real-time analytics on the operational data.'\n   - Optional row to skip if running short - but it lands the HTAP advantage if a data team person is in the room.\n\nLAND (read it slowly):\n'Same four capabilities every RAG tutorial tells you to wire up. One operational data layer. One ACID transaction. One MySQL wire.'\n\nKEY POINT TO LAND: This slide makes 4 -> 1 specific. The audience now has named features they can ask about. The Pinecone budget conversation is over.\n\nBRIDGE TO TIDB CLOUD: 'And the platform that delivers all five of those capabilities is built specifically for this. Let me show you the architecture underneath.'",
  ...
  tidbx:
    "[~3 min] TIDB CLOUD ARCHITECTURE. The visual platform credibility slide - shows the actual layered architecture, top to bottom. Walk it like you're sketching it on a whiteboard.\n\nOPEN: 'TiDB has been in production 11 years. The cloud architecture is the rebuild for the AI era - same MySQL wire your team already speaks, completely different economics underneath. Let me walk the layers.'\n\nWALK THE STACK - top to bottom, ~25 sec each:\n\nLAYER 1 - YOUR APPS (top)\n   - 'MySQL wire protocol. No driver changes. Your existing apps don't know they're talking to a distributed system.'\n   - Anchor the spine: TiDB doesn't disrupt the team's fluency.\n\nLAYER 2 - STATELESS SQL COMPUTE\n   - 'Compute is stateless. Auto-scales in seconds, not days. Pay-as-you-go - you don't reserve for peak.'\n   - 'And THIS is where vectors, BM25, hybrid search, and SQL all live - one query layer, four modalities.'\n\nLAYER 3 - ROW ENGINE + COLUMNAR ENGINE (side by side)\n   - 'Row engine handles transactions, vector lookups, full-text search. Columnar engine - same data, replicated columnar - handles real-time analytics. Zero ETL between them.'\n   - 'Your operational data and your analytics live in one cluster.'\n\nLAYER 4 - SHARED SERVICES, ISOLATED LANE\n   - 'Compaction, analyze, DDL, index builds - they all run in a separate microservice lane. They never disturb online traffic.'\n   - For the room: 'This is the answer to your platform team's question - what happens when AI vector indexing kicks in. It doesn't take down your OLTP.'\n\nLAYER 5 - S3 / GCS / AZURE BLOB (the foundation)\n   - 'Object storage is the source of truth. Not block storage. Cloud-economic by design - up to 80 percent lower cost on cold-data-heavy estates. THIS is the line your CFO will care about most.'\n\nLAND: 'One unified platform - OLTP, OLAP, Gen-AI workloads. Same MySQL fluency. New cloud economics.'\n\nKEY POINT TO LAND: Audience walks away with a mental picture of TiDB Cloud's architecture. Not a 2015 distributed SQL story - a 2026 cloud-native rebuild that's why all the consolidation customers work.\n\nBRIDGE TO BYOC: 'And for the regulated CIOs in the room - one quick note on how this gets deployed safely.'",
  ...
  byoc:
    "[~2 min] BYOC - data plane vs control plane diagram. Walk the two boxes. The whole slide answers ONE question: where does my data live, and what does PingCAP touch?\n\nOPEN: 'One question I get from every regulated CIO: where does my data actually live, and what does the vendor touch? With BYOC the answer is on screen - two planes, one boundary.'\n\nWALK LEFT BOX - DATA PLANE (your cloud account) ~45 sec:\n   - 'Everything in the left box runs in YOUR AWS, Azure, or GCP account. Your VPC, your subnets, your security groups.'\n   - Walk down the stack: 'TiDB cluster - the SQL layer, TiKV row store, TiFlash columnar, PD. Object storage - S3 / GCS / Blob - source of truth, in your bucket. IAM - 13 static roles, all auditable. KMS - your keys, BYOK encryption at rest.'\n   - Land: 'All your data. Your encryption. Your network. Your auditors.'\n\nWALK MIDDLE - PRIVATELINK ~15 sec:\n   - 'The two planes connect via PrivateLink. Critical detail: this is management traffic ONLY. No customer data crosses this connection. Ever.'\n   - This is the line that closes most security objections.\n\nWALK RIGHT BOX - CONTROL PLANE (PingCAP managed) ~45 sec:\n   - 'Everything in the right box is what PingCAP runs on your behalf - centralized so you don't have to staff a TiDB ops team.'\n   - Walk down the stack: 'Cluster orchestration - provisioning, upgrades, scaling. Monitoring - we see metrics and health, never row data. Backup coordination - we trigger backups, but the backups stay in YOUR bucket. Audit log of every operation, customer-readable. And the web console plus API - SSO and RBAC.'\n   - Land: 'We see metrics, never data. Your CISO signs off, not us.'\n\nFOOTER LAND: 'SOC 2 Type II. HIPAA-eligible. 3-4 hour deploy. Healthcare, financial services, government - this is the deployment they need.'\n\nKEY POINT TO LAND: The diagram is the answer. Don't paraphrase - point at it. The two-plane separation is the security story.\n\nBRIDGE TO MEMORIES: 'OK - architecture is built, deployment is safe. Now let's talk about the workload that drives most of this conversation: agent memory.'",
