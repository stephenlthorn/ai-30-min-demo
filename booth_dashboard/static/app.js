...
const SLIDES = [
  ...
  { id: "convergence", label: "Customer Proof",   shortLabel: "4" },
  { id: "sarah",       label: "Urgent Example",   shortLabel: "5" },
  { id: "why",         label: "Architecture",     shortLabel: "6" },
...
const SPEAKER_NOTES = {
  title:
    "[~2 min] OPEN COLD. Don't say hello. Don't thank anyone. The first sentence is your hello.\n\n70/30 REWEIGHTING: lead with the engineering-effort question. AI is the trigger, not the story.\n\nVERBATIM OPENING:\n\n'By the end of the next 45 minutes you'll be able to walk into your Monday staff meeting and ask the one question that tells you whether your data infrastructure is building your business - or being maintained by it.\n\n*(pause)*\n\nHow much of our engineering team's calendar this quarter is data infrastructure plumbing instead of building product?\n\n*(pause)*\n\nIndustry surveys say 40 to 60 percent at most enterprises. Sharding strategies. Replication topology. Sync jobs that broke at 3 AM. Schema migrations across a dozen services. Warehouse ETL. Vector store reindexing. Not building product. Plumbing.\n\n*(slow down)*\n\nAI is the trigger that brought us into this room. Every AI pilot you've seen adds another four systems on top of what you already have. The same architectural sprawl that's been quietly draining your team for a decade is about to compound.\n\nThe companies winning aren't doing it with smarter AI. They're doing it with simpler infrastructure.\n\nYour engineers should build the business. Not the plumbing.'\n\nThen About slide.",
  ...
  "simplify":
    "[~6 min] THE SPINE - hero slide, expanded. Sell it hard.\n\nKEY ARC:\n- 40-60% of senior engineering effort goes to plumbing - sharding, replication, sync jobs, schema migrations, vector store reindexing, warehouse ETL.\n- Count the systems your team operates today - sharded transactional fleet (5-10), read replicas, analytics warehouse + ETL, search index, cache cluster. 5-12 separate operational systems before AI.\n- Customers don't care about replication topology. AI-native winners aren't smarter - they spend engineering on the business.\n\nTHEN ESCALATE TO AI:\n- Every AI pilot adds 4 more systems: vector store, search index, embedding pipeline, orchestration glue.\n- 50% becomes 70%.\n\nPLANT THE PROOF POINTS (these get expanded next slide):\n- Pinterest: 6 → 1, 80% cost reduction\n- Flipkart: 700+ MySQL clusters → 1\n- Plaid: 26 engineer-weeks eliminated in one quarter\n- Dify: 500K+ containers → 1\n\nKEY LINE: 'The question isn't whether you can afford another four systems for AI. It's whether you can afford the ones you already have.'\n\nBRIDGE TO CONVERGENCE: 'Let me show you the math on those four customers - and what their engineering teams went back to building.'",

  convergence:
    "[~6 min] CUSTOMER PROOF - moved up, expanded. This is the proof of the SIMPLIFICATION SPINE, not just AI.\n\nLEAD WITH NON-AI CUSTOMERS (the headline is engineering, not QPS):\n\n- PINTEREST: 6 → 1. 80% infra cost reduction. 3-5x p99 latency improvement. 1.3M+ QPS. The headline is the engineers freed, not the QPS.\n- FLIPKART: 700+ MySQL clusters → 1 unified TiDB. P95 <5ms at 1M+ QPS. Order-of-magnitude less to operate.\n- PLAID: 104 minutes of downtime + 26 engineer-weeks eliminated in one quarter. Two engineers freed for an entire quarter.\n- BOLT: 3x storage compression - less infrastructure to operate.\n- GMGN: 2 weeks to migrate, 50% cost reduction.\n\nLINKEDIN QUOTE: 'Memory stops being incidental context and becomes a first-class primitive with explicit read/write semantics and lifecycle management.' Language of operating an infrastructure layer, not bolting on a feature.\n\nTHEN AI-NATIVE (secondary):\n- DIFY: 500K+ containers → 1. 90% ops reduction.\n- MANUS: millions of agent branches on one cluster. $5/month/user pricing because per-DB cost scales to zero.\n- TWO TOP-10 AI LABS (NDA) picked the same architecture for the same reason.\n\nLAND: 'Two journeys. One architecture. Enterprise from data platform consolidation. AI from headcount constraint. Same place.'\n\nREINFORCE THE SPINE: 'The headline is never we got more QPS. The headline is we freed our engineers.'\n\nBRIDGE TO SARAH: 'You don't have a Pinterest-scale problem yet. But there's one workload that's about to put you there faster than anything else. Let me show you the most urgent example.'",

  sarah:
    "[~3 min] THE URGENT EXAMPLE - compressed. Reframed: AI is the SYMPTOM that proves the existing complexity is now unsurvivable. Pick ONE vignette.\n\nWALK THE CHAT TABS:\n- Sarah - $4,800 VIP from 3 weeks ago. Tonight the agent has no idea who she is. Same script. Same stranger.\n\nNAME THE THREE-VICTIM IMPACT:\n- Customer: VIP treated like a stranger\n- Operations: support escalation queue grows\n- Engineering: 6 weeks of glue code, half doesn't work\n\nTHE THREE FAILURE MODES (read each):\n- NEVER WRITTEN\n- NOT FINDABLE\n- CONTEXT ROT\n\nKEY LINE: 'No memory - by design. This is not a model problem. It's a data architecture problem. Your enterprise has solved data architecture problems before. The companies on the previous slide solved this exact one - they just solved it before AI made it urgent.'\n\nBRIDGE TO ANSWER: 'Here's what the consolidated answer actually looks like.'",

  why:
    "[~3 min] THE ARCHITECTURE - compressed. 9-12 → 1.\n\nLEFT (Fragmented):\n- 4 systems duct-taped: SQL DB, Vector DB, Analytics DB, Search DB.\n- Agent at the top: 'which version of reality is correct?'\n- THIS IS ON TOP OF: sharded transactional fleet (5-10), read replicas, analytics warehouse + ETL.\n- Count the systems - you're at 9-12, not 4.\n\nRIGHT (TiDB):\n- One operational data layer. MySQL wire protocol. One transaction. One audit log.\n- ONE TEAM operates ONE system instead of nine.\n\nLAND: 'Memory is infrastructure. So is the data layer underneath it. Neither belongs in nine systems.'\n\nBRIDGE TO MEMORIES: 'Three named patterns to watch for in the demo.'",
