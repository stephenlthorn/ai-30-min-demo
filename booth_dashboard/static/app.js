...
const SPEAKER_NOTES = {
  ...
  sarah:
    "[~3 min] THE URGENT EXAMPLE - audience-mapping pattern. Three failure modes (Never written / Not findable / Context rot) each anchored to an industry the room recognizes. The point: same architecture failure, three different verticals, ONE root cause.\n\nOPEN: 'Different vertical. Different scenario. Same root cause.' (read the subtitle off the slide)\n\nWALK THE THREE FAILURE MODES - one beat each, ~30-40 sec:\n\n01 - NEVER WRITTEN | Retail\n   - Read the scenario: 'Top-1% VIP returned a $4,800 order three weeks ago. Tonight she's back to reorder. The agent has no record - asks her to start over.'\n   - Cause: the conversation was never persisted. Either no episodic store at all, or there is one but the agent never writes to it.\n   - Tie back: 'Anyone in this room running pilot agents can predict this exact failure within 90 days of launch.'\n\n02 - NOT FINDABLE | Healthcare\n   - Read the scenario: 'Patient's allergy is in the EHR. The booking agent can't reach it - offers an appointment with a contraindicated specialist.'\n   - Cause: the data exists - in a separate system. The agent retrieves only from what it indexed (the document store), not the operational system of record.\n   - Sharper version (if a healthcare CIO is in the room): 'This is the same architecture failure that breaks every vendor agent demo - the data is there, it's just not where the agent looked.'\n\n03 - CONTEXT ROT | B2B SaaS\n   - Read the scenario: 'Renewal agent loads 2M tokens of account history. Buried in it: the customer churned from this exact product six months ago. Agent recommends it again.'\n   - Cause: bigger context window is a bigger meter, not a better answer. Without curation, the agent reads everything and surfaces the wrong thing.\n   - Cost tie-in: 'This is the slide that turns into the Token Tax slide later - context rot is what bigger context windows cost you AND what they fail to fix.'\n\nLAND (read it slowly):\n'Three patterns. One root cause: agent memory split across four systems that don't share a transaction.'\n\nKEY POINT TO LAND: Each card is a different industry, but the cause column tells you the architecture is identical. The room sees themselves in at least one card - and the cause column proves it's the architecture, not the industry.\n\nBRIDGE TO MEMORIES: 'So what does the answer look like? Three patterns of memory the agent needs - and TiDB makes all three live in one ACID query.'",
