...
const SLIDES = [
  ...
  { id: "sarah",       label: "The Hook",         shortLabel: "4" },
  { id: "why",         label: "The Answer",       shortLabel: "5" },
  { id: "convergence", label: "Convergence",      shortLabel: "6" },
  ...
  { id: "manus",       label: "Manus Math",       shortLabel: "10" },
...
const SPEAKER_NOTES = {
  ...
  "about-tidb":
    "[~3 min] LEAD WITH SIMPLIFICATION, not consolidation. The audience knows the scale-out pain - sharding, replication topology, ops engineers. Logos prove TiDB freed those engineers.\n\nKEY ARC:\n- The MySQL/Postgres scale-out moment - sharding strategies, multi-reader/single-writer topology, replication lag, expert engineers as a team and a budget line.\n- TiDB does that scale-out automatically. MySQL wire protocol, distributed SQL underneath. From Spanner papers, 2015. 11 years in production.\n- Pinterest didn't just consolidate six systems - they freed the engineers who maintained them.\n- LinkedIn runs it at scale because more sharding experts isn't viable.\n- Plaid eliminated 26 engineer-weeks of toil in one quarter.\n\nTHEN ADD AI:\n- Manus, Dify, two labs under NDA - found TiDB from a different direction. They never had a sharded MySQL fleet. They architected for simplicity from day one because they couldn't afford your infrastructure team.\n\nLAND: 'Same architecture, two journeys. Enterprise from data platform consolidation. AI from cost-of-state. They met in the same place.'\n\nBRIDGE TO SIMPLIFY: 'Before I show you the AI failure mode, let's look at where your engineering team actually spends its time today.'",

  "simplify":
    "[~2 min] NEW BEAT. The spine of the dual thesis. Forces the audience to recognize they already have the problem AI is about to compound.\n\nKEY ARC:\n- 40-60% of senior engineering effort at most enterprises goes to data infrastructure plumbing - sharding, replication, sync jobs, schema migrations, vector store reindexing, warehouse ETL.\n- Not building product. Plumbing.\n- Customers don't care about replication topology. AI-native startups winning aren't smarter - they spend engineering on the business, not the plumbing.\n- AI adds 4 more systems on top of what you already have. 50% becomes 70%.\n\nKEY LINE: 'The question isn't whether you can afford another four systems. It's whether you can afford to keep the ones you already have.'\n\nBRIDGE TO SARAH: 'Let me show you what that looks like in a customer interaction your audience cares about.'",

  sarah:
    "[~4 min] THREE VICTIMS of the same failure mode: customer, operations, engineering. Slow down between each vignette.\n\nVIGNETTE 1 - RETAIL: Sarah is a $4,800 VIP from three weeks ago. Tonight the agent has no idea who she is. Walk the tabs.\n\nVIGNETTE 2 - OPERATIONS: distributor calls Taipei AM, Shanghai PM. Same person. Same open ticket. Two strangers.\n\nVIGNETTE 3 - ENGINEERING: team wanted to ship the agent in one sprint. Spent six weeks wiring vector store to transactional DB, building sync jobs, debugging consistency at 3 AM. Shipped in week eight. Half doesn't work.\n\nKEY LINE: 'Same failure mode. Three different victims. Customer experience, operational reality, engineering velocity. All three are bleeding.'\n\nLAND ON THE THREE FAILURE MODES - read each label out loud:\n- NEVER WRITTEN - the agent had context, didn't persist it.\n- NOT FINDABLE - it's in the database somewhere, can't retrieve it.\n- CONTEXT ROT - it's there, but it's stale or contradictory.\n\nLAND: 'This is how LLMs work. No memory - by design. It's not a model problem - it's a data architecture problem. Your enterprise has solved data architecture problems before.'\n\nBRIDGE TO ANSWER: 'Here's what the architectural answer looks like - and why your existing complexity is part of the answer, not a separate problem.'",

  why:
    "[~5 min] THE ANSWER. SHOW THE COMPOUNDING - the AI stack adds 4 systems on top of what's already there.\n\nLEFT (Fragmented stack):\n- Four systems duct-taped together: SQL DB, Vector DB, Analytics DB, Search DB.\n- The agent at the top: 'which version of reality is correct?'\n- Real-world sync issues: 'Sync broke overnight,' 'Recommendations gone stale,' 'Customer delete missed.'\n- THEN ESCALATE: this 4-system AI stack lives ON TOP of your already-sharded transactional fleet, read replicas, analytics warehouse with ETL. Count the systems - you're not at 4. You're at 9 or 12.\n- Each one needs an engineer who knows it cold. Each one fails differently. Each one is a 3 AM page.\n\nRIGHT (TiDB):\n- Same capabilities. One operational data layer. MySQL wire protocol. One transaction. One audit log. ONE TEAM operates ONE system instead of nine.\n\nLAND - read both lines:\n- 'Infrastructure designed for legacy transactional purposes, not for compound knowledge.'\n- 'Memory is infrastructure - not a feature you bolt on. And neither is the infrastructure underneath it.'\n\nBRIDGE TO CONVERGENCE: 'And it's not just us saying this - here's who's already picked this architecture, and what it bought them.'",

  convergence:
    "[~3 min] LEAD WITH ENGINEERING EFFORT - not QPS. The headline is what these companies got back, not how much they processed.\n\nKEY ARC:\n- Pinterest: 6 systems consolidated, 80% infra reduction, BUT THE STORY is the ops team that went small and went back to building product.\n- Plaid: 26 engineer-weeks of toil eliminated in one quarter.\n- LinkedIn quote: 'Memory stops being incidental context and becomes a first-class primitive...' - the LANGUAGE OF OPERATING infrastructure, not bolting on a feature.\n\nTHEN PIVOT TO AI-NATIVE - they converged because they couldn't AFFORD the engineering team:\n- Dify: 500K+ containers, 90% ops reduction\n- Manus: millions of agent branches, $5/month/user pricing\n- Two top-10 AI labs (anonymized under NDA)\n\nLAND: 'Two journeys, one architecture. Enterprise from data platform consolidation. AI from headcount constraint. Same place.'\n\nBRIDGE TO MEMORIES: 'So what IS that architecture? Three named memory types - all in one database, not three more systems for your team to operate.'",
  ...
  whynow:
    "[~3 min] CLOSING. Promise kept. FOUR PILLARS, not three.\n\nFIRST - read the title quartet on screen, slowly:\n'The model forgets. The platform remembers. The human decides. The engineers build the business.'\n\nThen walk the four columns:\n- THE MODEL forgets - stateless by design, every session starts from zero.\n- THE PLATFORM remembers - three memories, one cluster, one ACID transaction.\n- THE HUMAN decides - engineers stop digging through logs, they curate the playbooks.\n- THE ENGINEERS BUILD - they're not maintaining nine systems anymore. They're shipping product.\n\nTHEN THE TWO MONDAY QUESTIONS (read SLOWLY):\n1. 'Where, exactly, does our agent's memory live? Show me the database.'\n2. 'How much of our engineering team's calendar this quarter is data infrastructure plumbing instead of business product?'\n\nThe tells:\n- If first answer is four systems and a sync job → you have a roadmap risk.\n- If second answer is more than 30% → you have a capacity problem AI is about to compound. The AI-native startup threatening you doesn't have it.\n\nLet that land. Don't fill the silence.\n\nFINAL LINES:\n'Memory isn't stored. It's maintained. Engineering is where you spend it.'\n\nThen point at the QR code: 'Find me at the booth - happy to map your current data architecture and tell you what consolidating it would free up. No pitch. Diagnostic only.'"
};
